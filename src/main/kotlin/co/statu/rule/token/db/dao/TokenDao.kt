package co.statu.rule.token.db.dao

import co.statu.rule.database.Dao
import co.statu.rule.token.db.model.Token
import co.statu.rule.token.type.TokenType
import io.vertx.jdbcclient.JDBCPool
import io.vertx.sqlclient.Pool
import java.util.*

abstract class TokenDao : Dao<Token>(Token::class) {

    abstract suspend fun add(
        token: Token,
        jdbcPool: Pool
    ): UUID

    abstract suspend fun isExistsByTokenAndType(
        token: String,
        tokenType: TokenType,
        jdbcPool: Pool
    ): Boolean

    abstract suspend fun deleteByToken(token: String, jdbcPool: Pool)

    abstract suspend fun deleteBySubject(subject: String, jdbcPool: Pool)

    abstract suspend fun deleteBySubjectAndType(subject: String, type: TokenType, jdbcPool: Pool)

    abstract suspend fun getByTokenSubjectAndType(
        token: String,
        subject: String,
        type: TokenType,
        jdbcPool: Pool
    ): Token?

    abstract suspend fun getByTokenAndSubject(
        token: String,
        subject: String,
        jdbcPool: Pool
    ): Token?

    abstract suspend fun getByToken(
        token: String,
        jdbcPool: Pool
    ): Token?

    abstract suspend fun getLastBySubjectAndType(
        subject: String,
        type: TokenType,
        jdbcPool: Pool
    ): Token?

    abstract suspend fun getAllBySubjectAndType(
        subject: String,
        type: TokenType,
        jdbcPool: Pool
    ): List<Token>

    abstract suspend fun deleteById(
        id: UUID,
        jdbcPool: Pool
    )
}